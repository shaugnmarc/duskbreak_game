...
func add_item(player_name: String, score_value: String) -> void:
	var score_container_left = $HBoxContainer/ContainerRect/Board/HighScores/ScoreItemContainer
	var score_container_right = $HBoxContainer/ContainerRect2/Board/HighScores/ScoreItemContainer
	...
	score_container_left.add_child(item)
	
	if score_container_left.get_child_count() < 10:
		score_container_left.add_child(item)
	elif score_container_left.get_child_count() == 10:
		score_container_right.add_child(item)
	elif score_container_right.get_child_count() == 12:
		pass

func add_no_scores_message() -> void:
	var item = $HBoxContainer/ContainerRect/Board/MessageContainer/TextMessage
	item.text = "No scores yet!"
	$HBoxContainer/ContainerRect/Board/MessageContainer.show()
	item.offset_top = 135
...
func hide_message() -> void:
	$HBoxContainer/ContainerRect/Board/MessageContainer.hide()


func clear_leaderboard() -> void:
	var score_item_container = $HBoxContainer/ContainerRect/Board/HighScores/ScoreItemContainer
